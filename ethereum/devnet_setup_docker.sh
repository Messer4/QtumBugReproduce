...
import_key () {
    ...
    until [ ! $i -lt 10 ]
    do
      res=$(curl --data-binary "{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", \"method\": \"importprivkey\", \"params\": [\"${1}\", \"address1\", false] }" -H 'content-type: text/plain;' http://qtum:testpasswd@qtum_testchain:3889)
...
curl --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "generatetoaddress", "params": [1000, "qUbxboqjBRp96j3La8D1RYkyqx5uQbJPoW"] }' -H 'content-type: text/plain;' http://qtum:testpasswd@qtum_testchain:3889

# address2
echo Seeding qLn9vqbr2Gx3TsVR9QyTVB5mrMoh4x43Uf
curl --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "generatetoaddress", "params": [1000, "qLn9vqbr2Gx3TsVR9QyTVB5mrMoh4x43Uf"] }' -H 'content-type: text/plain;' http://qtum:testpasswd@qtum_testchain:3889

# address3
echo Seeding qTCCy8qy7pW94EApdoBjYc1vQ2w68UnXPi
curl --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "generatetoaddress", "params": [500, "qTCCy8qy7pW94EApdoBjYc1vQ2w68UnXPi"] }' -H 'content-type: text/plain;' http://qtum:testpasswd@qtum_testchain:3889

# address4
echo Seeding qWMi6ne9mDQFatRGejxdDYVUV9rQVkAFGp
curl --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "generatetoaddress", "params": [250, "qWMi6ne9mDQFatRGejxdDYVUV9rQVkAFGp"] }' -H 'content-type: text/plain;' http://qtum:testpasswd@qtum_testchain:3889

# address5
echo Seeding qLcshhsRS6HKeTKRYFdpXnGVZxw96QQcfm
curl --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "generatetoaddress", "params": [100, "qLcshhsRS6HKeTKRYFdpXnGVZxw96QQcfm"] }' -H 'content-type: text/plain;' http://qtum:testpasswd@qtum_testchain:3889

# address6
echo Seeding qW28njWueNpBXYWj2KDmtFG2gbLeALeHfV
curl --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "generatetoaddress", "params": [100, "qW28njWueNpBXYWj2KDmtFG2gbLeALeHfV"] }' -H 'content-type: text/plain;' http://qtum:testpasswd@qtum_testchain:3889
